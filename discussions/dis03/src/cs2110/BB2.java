 ...
 * extents.
 */
public class BB2 implements BoundingBox {
    ...
    public BB2(Point lower, Point upper) {
        this(new Point(0.5*(lower.x() + upper.x()), 0.5*(lower.y() + upper.y())),
                upper.x() - lower.x(), upper.y() - lower.y());
    }

    public BB2(Point center, double width, double height) {
        assert center != null;
    ...
    public BoundingBox intersect(BoundingBox b) {
        ...
        if ((xMin > xMax) || (yMin > yMax)) {
            ...
            return new BB2(new Point(), 0.0, 0.0);
        }
        // Convert to center+extents representation.
        return new BB2(new Point(0.5 * (xMin + xMax), 0.5 * (yMin + yMax)),
                xMax - xMin, yMax - yMin);
    }
    ...
    public BoundingBox scale(double s) {
        ...
        // Scale extents;
        return new BB2(center, s * width, s * height);
    }
    ...
    public BoundingBox shifted(double dx, double dy) {
        ...
        // Shift center.
        return new BB2(center.shifted(dx, dy), width, height);
    }
