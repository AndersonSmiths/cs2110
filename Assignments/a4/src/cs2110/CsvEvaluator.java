...
public class CsvEvaluator {
    ...
    public static void evaluateCsv(CSVParser parser, CSVPrinter printer) throws IOException {
        ...
        for (CSVRecord r : parser) {
            ...
            int nCols = 1;
            for (String cell : r) {
                // initializing output to handle 'cannot resolve symbol'
                String output = new String();
                // determining cell naming
                String letter = colToLetters(nCols);
                // cell name is column letter + row number
                String cellName = letter + nRows;

                // replacing formula cells with evaluated value
                if (cell.startsWith("=")) {
                    try {
                        //The remainder of the cell is interpreted as a formula
                        //in Reverse Polish Notation with each token separated by whitespace.
                        String function = cell.substring(1);
                        Expression calc = RpnParser.parse(function, defs);
                        double result = calc.eval(vars);
                        // handling formula referencing a cell that does not contain a number
                        if (Double.isNaN(result)) {
                           output = "#N/A";
                        } else {
                            // mapping coordinate of cell
                            vars.set(cellName, result);
                ...
                // handling non formula cells
                } else {
                    try {
                        double floatingPoint = Double.parseDouble(cell);
                        // mapping coordinate of cell
                        vars.set(cellName, floatingPoint);
                        output = cell;
                    } catch (NumberFormatException e) {
                        vars.set(cellName, Double.NaN);
                        output = cell;
                    }
                }
            ...
            nRows++;
            // called to finish the current row being written and move to next row
            printer.println();
